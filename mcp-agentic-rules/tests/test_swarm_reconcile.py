...
def test_plan_seeds_only_uncoordinated_issues():
    ...
    seeds = sorted(a["issue"] for a in actions if a["action"] == "seed")
    assert seeds == [1, 4], "exactly the label-less issues get re-seeded"
...
def test_dry_run_logs_but_never_mutates(capsys):
    ...
    assert all(line["dry_run"] for line in lines) and len(lines) == 4
