...
def find_project_root(start: Path = None) -> Optional[Path]:
    ...
    # If mcp-agentic-rules is inside a project, that's likely the project we want.
    if mcp_root_env:
        mcp_root = Path(mcp_root_env).resolve()

        # Check parent of mcp-agentic-rules (common case)
...
def get_mcp_dir(root: Optional[Path] = None) -> Path:
    """
    Get the MCP storage directory (supports partitioning via MCP_WORKSPACE).
    """
    if root is None:
        root = find_project_root() or Path.cwd()
    workspace = os.environ.get('MCP_WORKSPACE')
    if workspace:
        clean_workspace = "".join(c for c in workspace if c.isalnum() or c in ('_', '-'))
        return root / '.mcp' / 'workspaces' / clean_workspace
    return root / '.mcp'
