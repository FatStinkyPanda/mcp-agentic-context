...
def main():
    ...
    # Analyze error. Only read stdin when a human is at a terminal or when
    # explicitly requested with --stdin: a silent open pipe (how agents
    # invoke tools) must fail fast instead of hanging forever.
    if args:
        error_text = ' '.join(args)
    elif '--stdin' in sys.argv or sys.stdin.isatty():
        ...
        Console.fail('No error text provided. Usage: mcp heal "<error text>" '
                     '(or --learn "lesson" / --lessons / --stdin)')
        return 1
