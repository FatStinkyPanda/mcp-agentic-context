...
def parse_file(path: Path) -> ParsedFile:
    ...
    # Fallback parsing if tree-sitter not available or failed to produce results
    if not result.functions and not result.classes:
        if language == 'python':
            result = _parse_python_fallback(path, source, result)
        else:
            result = _parse_generic_fallback(path, source, result, language)
...
def find_block_end(lines: List[str], start_line_idx: int) -> int:
    """Find the line index where the matching brace closes."""
    brace_count = 0
    started = False
...
def _parse_generic_fallback(path: Path, source: bytes, result: ParsedFile, language: str) -> ParsedFile:
    """Fallback parser for curly-brace languages using regex and brace tracking."""
    import re
    ...
    if language in ('javascript', 'typescript'):
        ...
        func_pattern = re.compile(r'\b(?:public|protected|private|static|\s)+\s+[\w<>,\[\]]+\s+([a-zA-Z_]\w*)\s*\([^)]*\)\s*(?:throws\s+[\w,\s]+)?\s*\{')
    elif language in ('c', 'cpp'):
        class_pattern = re.compile(r'\b(?:struct|class|union)\s+([a-zA-Z_]\w*)')
        func_pattern = re.compile(r'^\s*[\w:*&<>]+(?:\s+[\w:*&<>]+)*\s+([a-zA-Z_]\w*)\s*\([^)]*\)\s*\{')
    ...
    for i, line in enumerate(lines, 1):
        ...
        if class_pattern:
            ...
            if match:
                name = match.group(1)
                line_end = find_block_end(lines, i - 1)
                result.classes.append(CodeItem(
                    name=name,
                    item_type='class',
                    line_start=i,
                    line_end=line_end,
        ...
        if func_pattern:
            ...
            if match:
                ...
                if name:
                    line_end = find_block_end(lines, i - 1)
                    result.functions.append(CodeItem(
                        name=name,
                        item_type='function',
                        line_start=i,
                        line_end=line_end,
