...
def main():
    ...
    if not args:
        Console.fail("Usage: mcp context <query> [path] [--tokens N] [--compress]")
        print("\nExamples:")
        print('  mcp context "authentication"')
        print('  mcp context "database connection" src/')
        print('  mcp context "api handler" --tokens 8000 --compress')
        return 1
    ...
    if compress:
        from .autocontext import compress_code_block
        for item in result.items:
            item.content = compress_code_block(item.content, max_lines=20, query=query)
