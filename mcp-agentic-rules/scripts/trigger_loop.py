...
def main():
    ...
    if not loop_config.ENABLE_AUTO_LOOP:
        Console.info("Auto-Dev Loop: Disabled (User controllable via mcp-agentic-rules/config/loop_config.py)")
        return 0

    # Locate the prompt file
    root = find_project_root() or Path.cwd()
    # Check standard location first
    prompt_path = root / "mcp-agentic-rules" / "prompts" / "auto_dev.md"
