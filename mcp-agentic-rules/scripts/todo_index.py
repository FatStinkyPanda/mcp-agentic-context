...
def rebuild_todo_index_summaries(index: dict):
    """Rebuild type, priority, and file summaries from items list."""
    index["total"] = len(index["items"])
    index["by_type"] = {}
    index["by_priority"] = {"1": 0, "2": 0, "3": 0}
    index["by_file"] = {}

    for item in index["items"]:
        # Count by type
        item_type = item["type"]
        if item_type not in index["by_type"]:
            index["by_type"][item_type] = 0
        index["by_type"][item_type] += 1

        # Count by priority
        p_str = str(item["priority"])
        if p_str not in index["by_priority"]:
            index["by_priority"][p_str] = 0
        index["by_priority"][p_str] += 1

        # Count by file
        item_file = item["file"]
        if item_file not in index["by_file"]:
            index["by_file"][item_file] = 0
        index["by_file"][item_file] += 1


def index_todos(root: Path = None, changed_files: List[Path] = None) -> Dict:
    """Build or update TODO index and save to disk."""
    ...
    loaded_index = None
    if changed_files is not None and index_path.exists():
        try:
            with open(index_path, 'r', encoding='utf-8') as f:
                loaded_index = json.load(f)
            Console.info(f"Updating TODO index incrementally for {len(changed_files)} changed files...")
        except Exception:
            Console.warn("Failed to load existing TODO index; falling back to full scan.")
            changed_files = None

    if loaded_index and changed_files is not None:
        # Convert changed_files paths to absolute or relative strings for comparison
        changed_paths = {str(Path(f).resolve()) for f in changed_files}

        # Evict old items belonging to changed files
        loaded_index["items"] = [item for item in loaded_index["items"] if str(Path(item["file"]).resolve()) not in changed_paths]

        # Scan only changed files
        new_todos = []
        for file_path in changed_files:
            if file_path.exists():
                new_todos.extend(scan_file(file_path))

        # Merge new items
        for todo in new_todos:
            loaded_index["items"].append(asdict(todo))

        # Rebuild statistics
        rebuild_todo_index_summaries(loaded_index)
        index = loaded_index
        Console.ok(f"Updated TODO index incrementally. Total now: {index['total']}")
    else:
        Console.info(f"Scanning all files for TODOs in {root}...")
        ...
        rebuild_todo_index_summaries(index)
    ...
    p_high = index['by_priority'].get('1', 0)
    p_med = index['by_priority'].get('2', 0)
    p_low = index['by_priority'].get('3', 0)
    Console.ok(f"Found {index['total']} TODOs ({p_high} high, {p_med} medium, {p_low} low)")
