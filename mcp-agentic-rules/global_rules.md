...
```bash
# First, find where mcp-agentic-rules is installed
find . -name "mcp.py" -path "*/mcp-agentic-rules/*" 2>/dev/null

# Then run commands using the full path, for example:
python3 ./mcp-agentic-rules/mcp.py <command>

# Or if installed elsewhere:
python3 /path/to/mcp-agentic-rules/mcp.py <command>
```
...
1. **Find and Run MCP Tools**
   ...
   MCP=$(find . -name "mcp.py" -path "*/mcp-agentic-rules/*" 2>/dev/null | head -1)
...
1. **Initialize Project** (if new):
   ```bash
   # From the mcp-agentic-rules directory:
