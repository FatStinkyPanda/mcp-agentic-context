...
def get_package_root():
    """
    Get the absolute path to the mcp-agentic-rules directory.
    ...
    # Check if mcp-agentic-rules is in current directory
    if (cwd / 'mcp-agentic-rules' / 'mcp.py').exists():
        return cwd / 'mcp-agentic-rules'

    # Check if we're inside mcp-agentic-rules
    if (cwd / 'mcp.py').exists() and (cwd / 'scripts').exists():
        return cwd

    # Check parent directories
    for parent in cwd.parents:
        if (parent / 'mcp-agentic-rules' / 'mcp.py').exists():
            return parent / 'mcp-agentic-rules'
...
if MCP_ROOT is None:
    print("[FAIL] Cannot find mcp-agentic-rules directory")
    print("Make sure you're running from a project with mcp-agentic-rules installed")
    sys.exit(1)
...
# =============================================================================
# PROJECT_ROOT is the parent directory of mcp-agentic-rules, which is where
# mcp-agentic-context was installed. All MCP operations must be confined within this
# boundary to prevent indexing/operating on files outside the project.

def get_project_root():
    """Determine the project root (parent of mcp-agentic-rules directory)."""
    ...
    # Default: parent of mcp-agentic-rules
