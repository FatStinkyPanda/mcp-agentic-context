...
class NSyncHandler(FileSystemEventHandler):
    ...
    def ensure_rules_links(self):
        ...
        mcp_source = Path("C:/MCP/Projects/mcp-agentic-rules") if os.name == 'nt' else Path("/home/<remote-user>/Projects/mcp-agentic-rules")

        for item in self.repo_path.iterdir():
            if item.is_dir() and not item.name.startswith("."):
                target = item / "mcp-agentic-rules"
...
def init_project(name: str):
    ...
    try:
        ...
        print(f"[OK] Linked mcp-agentic-rules to {mcp_target}")
    except Exception as e:
    ...
    with open(project_path / ".cursorrules", "w") as f:
        ...
        f.write("- [Global Rules](mcp-agentic-rules/global_rules.md)\n\n")
        f.write("KEY CONTEXT:\n")
...
def setup_hooks():
    ...
    if os.name == 'nt':
        ...
        with open(post_commit_path, "w") as f:
            f.write(f"#!/bin/bash\npython3 /home/<remote-user>/Projects/mcp-agentic-rules/mcp.py nsync sync\n")
    ...
    if os.name == 'nt':
        ...
        with open(post_merge_path, "w") as f:
            f.write(f"#!/bin/bash\npython3 /home/<remote-user>/Projects/mcp-agentic-rules/mcp.py index-all\n")
